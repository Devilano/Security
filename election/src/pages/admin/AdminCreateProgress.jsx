...
const AdminCreateProgress = () => {
    ...
    const changeNTtitle = (e) => {
        setNewsTitle(DOMPurify.sanitize(e.target.value));
    }

    const changePublisher = (e) => {
        setPublisher(DOMPurify.sanitize(e.target.value));
    }

    const changeSrcFrom = (e) => {
        setSourceFrom(DOMPurify.sanitize(e.target.value));
    }

    const changeDDate = (e) => {
        setDate(DOMPurify.sanitize(e.target.value));
    }
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // form data is not needed here in react but for image no needed
        ...
        createProgressApi(formData).then((res) => {
            if (res.data.success === false) {
                ...
                window.location.reload(); // Reload the page to reflect new progress
            }
        }).catch(e => {
